...
while(<IN>) {
    ...
    if ($junc11_strand eq "+") {
        ...
        print STDERR $samtools_path . "/samtools view " . $seq . " " . $junc11_chr . ":" . $start . "-" . $end . " > " . $tempDir . "/temp_junc.sam" . "\n";
        my $ret = system($samtools_path . "/samtools view " . $seq . " " . $junc11_chr . ":" . $start . "-" . $end . " > " . $tempDir . "/temp_junc.sam");
        if ($ret != 0) {
    ...
    open(IN1, $tempDir . "/temp_junc.sam") || die "cannot open $!";
    while(<IN1>) {
        s/[\r\n]//g;
        my @F = split("\t", $_);
    ...
    if ($junc21_strand eq "+") {
        ...
        print STDERR $samtools_path . "/samtools view " . $seq . " " . $junc21_chr . ":" . $start . "-" . $end . " > " . $tempDir . "/temp_junc.sam" . "\n";
        my $ret = system($samtools_path . "/samtools view " . $seq . " " . $junc21_chr . ":" . $start . "-" . $end . " > " . $tempDir . "/temp_junc.sam");
        if ($ret != 0) {
            print STDERR "samtools error";
            exit $ret;
        }
    ...
    open(IN2, $tempDir . "/temp_junc.sam") || die "cannot open $!";
    while(<IN2>) {
        s/[\r\n]//g;
        my @F = split("\t", $_);
        my @flags = split("", sprintf("%011b", $F[1]));
...
system("rm " . $tempDir . "/temp_junc.sam");
